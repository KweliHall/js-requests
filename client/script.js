...
sayHelloButton.addEventListener("mouseout", backToNormal);
// PROBLEM 4
/*
...
const ohMy = () => {
    axios.get("http://localhost:300/animals").then(response => {
        console.log(response.data);
        for (let i = 0; i < response.data.length; i++) {
            let j = document.body.appendChild("j");
            document.body.appendChild(j);
            .testContent = response.data[i];
        }
        });
}
...
const repeatMyParam = () => {
    axios.get("http://localhost:300/repeat/orange").then(responce => {
...
// CODE HERE
let request = () => {
    axios.get("http://localhost:3000/query-test/?myname").then(response => {
        console.log(response.data)
    })
}
